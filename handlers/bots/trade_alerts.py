...
def _merge_filters(existing: Optional[Dict[str, List[str]]],
                   new: Optional[Dict[str, List[str]]]) -> Dict[str, List[str]]:
...
def _format_trade_alert(trade: Dict[str, Any], ts: Optional[datetime]) -> str:
    ...
    amount_str = format_amount(amount) if amount is not None else str(trade.get("amount", ""))
    price_str = format_price(price) if price is not None else str(trade.get("price", ""))
...
def build_trade_filters(configs: List[Dict[str, Any]],
                        controller_ids: List[str],
                        credentials_profile: Optional[str] = None) -> Dict[str, List[str]]:
...
async def handle_toggle_trade_alerts(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
